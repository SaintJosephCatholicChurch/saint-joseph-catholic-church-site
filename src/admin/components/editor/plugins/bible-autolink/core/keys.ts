...
const parseCurrentLine = (editor: Editor, offset: number): ParseResult | null => {
  ...
  if (matches && matches.length === 5) {
    ...
    if (finalRangeText === matches[1]) {
      const book = abbreviationsToUJSCCBBook[String(matches[2]).toLowerCase()];
      const verse = matches.length > 3 && hasText(matches[4]) ? `?${matches[4]}` : '';
      return {
        rng: newRng,
  ...
  if (additionalMatches && additionalMatches.length === 4) {
    ...
    const book = abbreviationsToUJSCCBBook[String(bookAndChapter.book).toLowerCase()];
    const chapter = additionalMatches[1] ?? bookAndChapter.chapter;
    const verse = hasText(additionalMatches[2]) ? `?${additionalMatches[2]}` : '';
