...
const setSize =
  (name: string, normalizeCss: CssNormalizer) =>
...
const getBorderStyle = (image: HTMLElement): string => image.style.borderStyle ?? '';

const isFigure = (elm: Node | null): elm is HTMLElement => Type.isNonNullable(elm) && elm.nodeName === 'FIGURE';

const isImage = (elm: Node): elm is HTMLImageElement => elm.nodeName === 'IMG';
...
const normalized =
  (set: (image: HTMLElement, value: string) => void, normalizeCss: CssNormalizer) =>
  (image: HTMLElement, name: string, value: string): void => {
    set(image, value);
    normalizeStyle(image, normalizeCss);
  };
