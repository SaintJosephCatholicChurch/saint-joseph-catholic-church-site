...
import { AdminHtmlEditor } from '../components/editor/AdminHtmlEditor';
...
const DOCUMENT_DETAIL_PANELS = ['editor', 'preview'] as const;

type DocumentDetailPanelId = (typeof DOCUMENT_DETAIL_PANELS)[number];
...
export function DocumentContentEditor({
  ...
  const [mobileDetailPanel, setMobileDetailPanel] = useAdminQueryParamState<DocumentDetailPanelId>({
    allowedValues: DOCUMENT_DETAIL_PANELS,
    defaultValue: 'editor',
    paramName: 'docPanel'
  });
                ...
                <Stack spacing={1.5}>
                  ...
                  {isMobileRecordLayout ? (
                    <AdminDetailTabs
                      ...
                      onChange={(nextPanel) => setMobileDetailPanel(nextPanel as DocumentDetailPanelId)}
                      tabs={[
      ...
      <Dialog fullWidth maxWidth="lg" onClose={() => setMediaPickerMode(null)} open={Boolean(mediaPickerMode)}>
        ...
        <DialogContent dividers>
          <AdminMediaLibrary
            ...
            currentAssetPublicPath={mediaPickerMode === 'hero-image' ? activePostDraft?.image || undefined : undefined}
            onChange={onSaved}
