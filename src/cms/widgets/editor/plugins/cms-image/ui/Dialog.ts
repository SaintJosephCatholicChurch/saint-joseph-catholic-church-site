...
const changeFileInput = (helpers: Helpers, info: ImageDialogInfo, state: ImageDialogState, api: API): void => {
  ...
  Arr.head<File>(data.fileinput).fold(
    () => {
      api.unblock();
    },
    (file) => {
...
const changeHandler =
  (helpers: Helpers, info: ImageDialogInfo, state: ImageDialogState) =>
  (api: API, evt: ChangeEvent): void => {
    if (evt.name === 'src') {
      changeSrc(helpers, info, state, api);
...
const makeDialogBody = (info: ImageDialogInfo): Ui.Dialog.TabPanelSpec | Ui.Dialog.PanelSpec => {
  if (info.hasAdvTab || info.hasUploadUrl || info.hasUploadHandler) {
    const tabPanel: Ui.Dialog.TabPanelSpec = {
      type: 'tabpanel',
      tabs: Arr.flatten([
    ...
    const panel: Ui.Dialog.PanelSpec = {
      type: 'panel',
...
const submitHandler =
  (editor: Editor, info: ImageDialogInfo, helpers: Helpers) =>
  (api: API): void => {
...
const createBlobCache =
  (editor: Editor) =>
  (file: File, blobUri: string, dataUrl: string): BlobInfo =>
...
const normalizeCss =
  (editor: Editor) =>
  (cssText: string | undefined): string =>
    doNormalizeCss(editor, cssText);

const parseStyle =
  (editor: Editor) =>
  (cssText: string): StyleMap =>
    editor.dom.parseStyle(cssText);

const serializeStyle =
  (editor: Editor) =>
  (stylesArg: StyleMap, name?: string): string =>
    editor.dom.serializeStyle(stylesArg, name);

const uploadImage =
  (_editor: Editor) =>
  (blobInfo: BlobInfo): Promise<UploadResult> =>
    tinymce.util.ImageUploader.upload([blobInfo], false).then((results) => {
      if (results.length === 0) {
...
export const Dialog = (editor: Editor): { open: () => void } => {
  ...
  const open = () => {
    collect(editor)
      .then((info: ImageDialogInfo): Ui.Dialog.DialogSpec<ImageDialogData> => {
