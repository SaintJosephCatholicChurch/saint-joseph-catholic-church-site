...
- Sessions: `localStorage` keys in `ADMIN_SESSION_KEYS`. `ADMIN_AUTH.siteId` in `adminConfig.ts` is empty in source; real OAuth needs it set.
...
8. Admin OAuth `siteId` is empty in source.
