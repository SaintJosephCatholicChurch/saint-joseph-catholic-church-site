...
2. `[ ]` Phase 2: Build pipeline and bundler add-ons
   ...
   Notes: `raw-loader` is still used by the CMS editor bundle, so any loader cleanup in this phase must preserve or replace that behavior deliberately.
3. `[ ]` Phase 3: MUI main stack from v5 to v6
   ...
   Notes: The direct `@mui/base` dependency has already been removed; if it still appears in the lockfile, treat it as transitive fallout from the remaining MUI stack until those packages are upgraded.
4. `[ ]` Phase 4: MUI main stack from v6 to v7
   ...
   Notes: This phase will affect both the public component library and the new admin shell, but admin-specific manual UX testing remains deferred until the final handoff phase.
