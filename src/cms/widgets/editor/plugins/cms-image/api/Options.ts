...
const register = (editor: Editor): void => {
  ...
  registerOption('image_list', {
    processor: (value) => {
      const valid =
        value === false || Type.isString(value) || Type.isArrayOf(value, Type.isObject) || Type.isFunction(value);
...
const hasUploadUrl = (editor: Editor): boolean => Strings.isNotEmpty(editor.options.get('images_upload_url'));

const hasUploadHandler = (editor: Editor): boolean => Type.isNonNullable(editor.options.get('images_upload_handler'));
