...
const sanitizer =
  (extractor: ListExtractor = getValue) =>
  (list: UserListItem[] | undefined | false): Optional<ListItem[]> => {
    if (list) {
...
const sanitize = (list: UserListItem[] | undefined): Optional<ListItem[]> => sanitizer(getValue)(list);
...
const findEntryDelegate = (list: ListItem[], value: string): Optional<ListValue> =>
  Arr.findMap(list, (item) => {
    ...
    } else if ((item as any).value === value) {
